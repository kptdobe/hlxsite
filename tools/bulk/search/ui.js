...
function displayResults(results, searchString) {
  ...
  if (!results || results.length === 0) {
    if (searchString) {
      ...
      child.innerHTML = `No results for "${searchString}"`;
    ...
    numberOfResults.innerHTML = `"${searchString}" has been found in ${results.length} files.`;
    ...
    results.forEach((r) => {
      ...
      for (let i=0; i < split.length;i++) {
        ...
        if (line.indexOf(searchString) !== -1) {
          ...
          text.innerHTML = line.replace(searchString, `<font>${searchString}</font>`);
 ...
 if (searchString !== '') {
    ...
    const promises = paths.map(async (path) => {
      ...
      if (c && c.content.indexOf(searchString) !== -1) {
        ...
        displayResults(searchResults, searchString);
      }
    });
    ...
    if (searchResults.length === 0) {
      displayResults(null, searchString);
    }
  }
}
...
function setListeners() {
  ...
  document.querySelector('#search input').addEventListener('keyup', debounce(search, 500));
}
